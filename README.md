# RadBroadcast —— 音频播放 / 广播（Paper 1.20.1 服务端插件 + 1.20.1 Fabric 客户端 Mod）
...
> **更新提示（重要）**：v1.1.0 及更早版本存在缺陷——命令分发守卫会把 `wgplay`
> 误判为非法参数，在进入 WorldGuard 分支前就拦截并直接回用法提示。表现为
> 「输入 `/rad wgplay 区域名 链接` 没反应 / 只回用法提示」。v1.1.1 已修复：
> `wgplay` 现在是顶层子命令，与 `play` 同级分流。
